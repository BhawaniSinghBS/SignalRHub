    ...
    public class TCPClient : IDisposable
    {
        ...
        {   //208.91.197.27 tcp // 2.travel safety
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!IPAddress.TryParse("tcpserver.com", out udpServerIpFromUrl))//tcpserver.com to connect
                    {
            ...
            catch (Exception ex)
            {
                ...
                lock (TCP.UDP.Constants.Constants.fileLockObject) // Lock the critical section to ensure thread safety
                {
                    try
                    {
                        System.IO.File.AppendAllText(TCP.UDP.Constants.Constants.LogFilePath, $"Reached  - {nameof(TCPClient)} Constructor : {nameof(TCPClient)} {DateTime.UtcNow} {Environment.NewLine}  Exception : \n {exs}");
                    }
                    catch (Exception ex2)
                    {
        ...
        public async Task<(int, byte[])> SendRequestToClientAndGetRawData(byte[] requestByteArray)
        {
            ...
            lock (TCP.UDP.Constants.Constants.fileLockObject) // Lock the critical section to ensure thread safety
            {
                try
                {
                    System.IO.File.AppendAllText(TCP.UDP.Constants.Constants.LogFilePath, $"Reached Class- {nameof(TcpClient)} Merthod  : {nameof(SendRequestToClientAndGetRawData)} {DateTime.UtcNow} \n");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"An error occurred while writing to the file: {ex.Message}");
                    ...
                    using (NetworkStream stream = TCPClientApp.GetStream())
                    {
                        ...
                        if (stream.CanRead)
                        {
                            ...
                            lock (TCP.UDP.Constants.Constants.fileLockObject) // Lock the critical section to ensure thread safety
                            {
                                try
                                {
                                    System.IO.File.AppendAllText(TCP.UDP.Constants.Constants.LogFilePath, $"Reached Class- {nameof(TcpClient)} Merthod  : {nameof(SendRequestToClientAndGetRawData)} \n wrote on tcp stream{DateTime.UtcNow} \n");
                                }
                                catch (Exception ex)
                                {
                                    Console.WriteLine($"An error occurred while writing to the file: {ex.Message}");
            ...
            catch (Exception ex)
            {
                ...
                lock (TCP.UDP.Constants.Constants.fileLockObject) // Lock the critical section to ensure thread safety
                {
                    try
                    {
                        ...
                        System.IO.File.AppendAllText(TCP.UDP.Constants.Constants.LogFilePath, $"Reached  - {nameof(TCPClient)} FunctionName : {nameof(SendRequestToClientAndGetRawData)} {DateTime.UtcNow} {Environment.NewLine}  Exception : \n {exceptionText}");
                    }
                    catch (Exception ex2)
                    {
                        // Console.WriteLine($"An error occurred while writing to the file: {ex.Message}");
